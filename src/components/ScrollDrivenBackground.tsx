...
const ScrollDrivenBackground = () => {
  ...
  return (
    <>
      ...
      {/* 1) Paper fiber texture - editorial magazine feel */}
      <div 
        className="fixed inset-0 pointer-events-none z-0 transition-opacity duration-700"
        style={{
          opacity: (1 - vignetteOpacity) * 0.04,
          backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 400 400' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='paper'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.7' numOctaves='6' stitchTiles='stitch'/%3E%3CfeColorMatrix type='saturate' values='0'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23paper)'/%3E%3C/svg%3E")`,
          mixBlendMode: 'multiply',
        }}
      />
      
      {/* 2) Aurora gradient waves - subtle pastel glow that flows */}
      <div 
      ...
      >
        {/* Primary aurora wave - pale gold */}
        <div 
          className="absolute w-[200%] h-[60%] blur-[100px] transition-all duration-[4000ms]"
          style={{
            background: `radial-gradient(ellipse 60% 40% at center, rgba(255, 245, 220, 0.25) 0%, transparent 70%)`,
            top: `${-10 + Math.sin(breathePhase * Math.PI * 2) * 8}%`,
            left: `${-50 + Math.sin((breathePhase + 0.3) * Math.PI * 2) * 15}%`,
            transform: `rotate(${-5 + Math.sin(breathePhase * Math.PI) * 3}deg)`,
          }}
        />
        {/* Secondary aurora wave - pale mint */}
        <div 
          className="absolute w-[180%] h-[50%] blur-[120px] transition-all duration-[5000ms]"
          style={{
            background: `radial-gradient(ellipse 50% 35% at center, rgba(220, 255, 245, 0.15) 0%, transparent 65%)`,
            bottom: `${-5 + Math.cos(breathePhase * Math.PI * 2) * 6}%`,
            right: `${-40 + Math.cos((breathePhase + 0.5) * Math.PI * 2) * 12}%`,
            transform: `rotate(${3 + Math.cos(breathePhase * Math.PI) * 2}deg)`,
          }}
        />
        {/* Tertiary aurora - pale rose */}
        <div 
          className="absolute w-[150%] h-[45%] blur-[90px] transition-all duration-[6000ms]"
          style={{
            background: `radial-gradient(ellipse 45% 30% at center, rgba(255, 235, 240, 0.12) 0%, transparent 60%)`,
            top: `${35 + Math.sin((breathePhase + 0.7) * Math.PI * 2) * 5}%`,
            left: `${-30 + Math.sin(breathePhase * Math.PI * 2) * 10}%`,
          }}
        />
      </div>
      
      {/* 3) Scattered constellation dots - pulsing stars */}
      <div 
        className="fixed inset-0 pointer-events-none z-0 transition-opacity duration-700"
        style={{
          opacity: 1 - vignetteOpacity,
      ...
      >
        {[...Array(25)].map((_, i) => {
          ...
          const pulsePhase = (breathePhase + delay) % 1;
          const opacity = baseOpacity + Math.sin(pulsePhase * Math.PI * 2) * 0.1;
          ...
          return (
            <div
              ...
              style={{
                ...
                left: `${x}%`,
                top: `${y}%`,
                backgroundColor: `rgba(120, 100, 80, ${opacity})`,
                boxShadow: `0 0 ${size * 2}px rgba(120, 100, 80, ${opacity * 0.5})`,
              }}
            />
          );
        })}
